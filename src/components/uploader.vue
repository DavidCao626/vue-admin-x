  ...
  <div class="imgUploader">
    <div class="file-list">
      ...
      <section class="file-item" v-if="imgStatus !== 'finished'">
        <div class="add">
          <span>+</span>
          <input type="file" pictype='30010003' multiple data-role="none" accept="image/*" @change="selectImgs" ref="file">
        </div>
      </section>
    </div>
    <div class="uploadBtn">
...
export default {
  ...
  computed: {
    ...mapGetters(['imgStore', 'imgPaths', 'imgStatus'])
  },
  methods: {
    // 选择图片
    selectImgs () {
...
.imgUploader .file-list .file-item {
  float: left;
  position: relative;
  width: 100px;
  text-align: center;
}
.imgUploader .file-list .file-item img {
  width: 80px;
  height: 80px;
  border: 1px solid #ececec;
}
.imgUploader .file-list .file-item .file-remove {
  position: absolute;
  right: 12px;
  display: none;
  top: 4px;
...
.imgUploader .file-list .file-item:hover .file-remove {
  display: inline;
}
.imgUploader .file-list .file-item .file-name {
  margin: 0;
  height: 40px;
...
.imgUploader .uploadBtn {
  position: relative;
}
.imgUploader .uploadBtn .empty {
  position: absolute;
  right: 0;
  bottom: 0;
